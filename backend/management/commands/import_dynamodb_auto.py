...
class Command(BaseCommand):
    ...
    def detect_table_name(self, table_path):
        ...
        # Method 1: Check manifest-files.json
        ...
        # Method 2: Check manifest-summary.json
        ...
        # Method 3: Check any .json file in root directory
        ...
        # Method 4: Try to infer from directory name patterns
        dir_name = os.path.basename(table_path)
        # Common patterns in DynamoDB exports
        if '-' in dir_name:
            # Sometimes exports have format like "tablename-timestamp-uuid"
            parts = dir_name.split('-')
            if len(parts) >= 2:
                potential_name = parts[0]
                # Check if this matches any known table
                known_tables = ['users', 'Groups', 'stock', 'stock_remarks', 
                              'production', 'push_to_production', 'stock_transactions', 'undo_actions',
                              'transactions', 'YourStockTableName', 'YourTransactionsTableName']
                if potential_name in known_tables:
            ...
            try:
                ...
                for item in items:
                    try:
                        ...
                        item_preview = str(converted_item)[:200] + '...' if len(str(converted_item)) > 200 else str(converted_item)
